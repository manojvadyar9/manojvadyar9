<h3 align="left">Hi there, I'm Vadyar Manoj! 👋</h3>

<p align="left">
I'm a passionate Game Developer and Game Design & Development student with experience in developing 2D and 3D games using Unity and C#. I enjoy creating engaging gameplay systems, designing game, and building interactive gaming experiences.
</p>

---

## 🎮 About Me

I am currently pursuing Game Design & Development at Image Creative Education and hold a B.Tech in Computer Science Engineering from Jawaharlal Nehru Technological University Anantapur.

I have experience developing multiplayer, 2D, and 3D games using Unity 6 and C#. My projects include combat systems, player movement, AI behaviors, UI systems, animation integration, and Photon Networking. I am passionate about game development, problem-solving, and continuously learning new technologies in the gaming industry.

---

## 💻 Technical Skills

### 🎯 Game Development
- 2D Game Development
- 3D Game Development
- Multiplayer Game Development
- UI Design & Implementation
- Combat Systems
- Enemy AI
- Player Controllers

### 🛠 Tools & Software
- Unity 6
- Maya
- Blender
- Adobe Photoshop
- Adobe Illustrator
- Adobe Substance 3D

### 💡 Programming Languages
- C#
- Unity C#
- UI Scripting (C#)
- Player Movement Scripting
- Enemy AI Programming

---

## 🚀 Projects

### ⚔️ Multiplayer Fighting Game
- Developed a multiplayer fighting game using Unity 6 and Photon Networking.
- Implemented combat mechanics, health systems, player movement, animations, and multiplayer synchronization.
- Integrated camera systems and networking features.

### 🏃 3D Endless Runner Game
- Built a 3D endless runner featuring:
  - Obstacle systems
  - Smooth camera movement
  - Environmental setup
  - Player controls
  - Gameplay mechanics

### 🔫 2D Shooting Game
- Developed a 2D shooting game with:
  - Shooting mechanics
  - Enemy AI systems
  - Collision detection
  - Player movement

---

## 📚 Education

### 🎓 Image Creative Education
**Game Art & Design**  
2024 – 2026

### 🎓 Jawaharlal Nehru Technological University Anantapur
**Bachelor of Technology (Computer Science Engineering)**  
2020 – 2025

---

## 🏆 Certifications

### Google UX Design Specialization
- Foundations of User Experience (UX) Design
- Start the UX Design Process
- Build Wireframes and Low-Fidelity Prototypes
- Conduct UX Research and Test Early Concepts
- Create High-Fidelity Designs and Prototypes in Figma
- Build Dynamic User Interfaces (UI)

---

## 🌱 Currently Learning

- Advanced Unity Game Development
- Multiplayer Systems & Photon Networking
- Unreal Engine 5
- Game AI & Enemy Behavior Systems
- Game Art & Environment Design
- UI/UX for Games

---

## 🤝 Open to Collaborate On

- Unity Game Development Projects
- Multiplayer Games
- Indie Game Development
- Game UI Design
- 2D & 3D Game Projects

---

## 📫 Connect With Me

- 📧 Email: manojvadyar@gmail.com
- 🎨 ArtStation: [Artstation Profile](https://www.artstation.com/myartstation/projects)
- 💼 LinkedIn: [LinkedIn Profile](https://www.linkedin.com/feed/)
- 🎭 Behance: [Behance Profile](https://www.behance.net/manojmanu90)
- 🌐 Portfolio: [itch.io](https://itch.io/dashboard)

---

## ⚡ Fun Fact

I enjoy turning game ideas into playable experiences and love solving challenging gameplay and programming problems. Whether it's combat systems, AI behavior, multiplayer networking, or game mechanics, I'm always excited to build something new! 🎮
